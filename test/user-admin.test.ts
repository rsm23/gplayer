...
describe('user administration routes', () => {
  ...
  it('renders user list and edit/new forms without password hashes or auth tokens', async () => {
    ...
    const legacyList = await app.inject({ method: 'GET', url: '/administrator/users/list/?q=admin', headers })
    expect(legacyList.statusCode).toBe(308)
    expect(legacyList.headers.location).toBe('/administrator/users/?q=admin')
