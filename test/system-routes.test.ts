...
describe('legacy-compatible system routes', () => {
  ...
  it('serves the Node landing page and its generator assets', async () => {
    ...
    expect(page.body).toContain('<title>GPlayer | One runtime. Every source.</title>')
    expect(page.body).not.toMatch(/<title[^>]*>[^<]*<!--/u)
    ...
    expect(page.body).toContain('data-share-network="facebook"')
    expect(page.body).toContain('data-share-network="telegram"')
    expect(page.body).toContain('data-share-more')
    expect(page.body).toContain('id="gotoTop"')
    ...
    expect(script.body).toContain('https://t.me/share/url')
    expect(script.body).toContain("typeof navigator.share === 'function'")
    expect(script.body).toContain('globalThis.scrollY <= 200')
    expect(script.body).toContain("behavior: reduced ? 'auto' : 'smooth'")
    ...
    expect(style.body).toContain('.public-share-rail')
    expect(style.body).toContain('.goto-top')
  })
  ...
  it('applies escaped Site Settings branding and colors to every public shell', async () => {
    ...
    for (const response of [dmca, missing]) {
      ...
      expect(response.body).toContain('data-share-network="facebook"')
      expect(response.body).toContain('data-share-network="telegram"')
      expect(response.body).toContain('data-share-more')
      expect(response.body).toContain('id="gotoTop"')
    }
  ...
  it('serves the sitemap alias and installable web-app assets', async () => {
    ...
    expect(worker.body).toContain("gplayer-node-public-v32")
