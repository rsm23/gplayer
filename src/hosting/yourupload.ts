...
export class YourUploadExtractor extends BaseExtractor {
  #loaded = false
  ...
  private async load(): Promise<void> {
    ...
    this.#loaded = true
    try {
      const response = await this.http.get({ url: `https://www.yourupload.com/embed/${encodeURIComponent(this.id)}` })
      if (response.status < 200 || response.status >= 300) return
      const file = between(response.body, 'video" content="', '"')
      if (!isHttpUrl(file)) return
      this.image = between(response.body, 'image" content="', '"')
      this.title = between(response.body, 'title" content="', '"')
