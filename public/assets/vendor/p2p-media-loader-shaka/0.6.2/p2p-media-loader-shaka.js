require = (function () {
...
})()(
	{
		1: [
			...
			{ "p2p-media-loader-shaka": "p2p-media-loader-shaka" },
		],
		2: [
			function (require, module, exports) {
				"use strict";
				...
				class Engine extends events_1.EventEmitter {
					constructor(settings = {}) {
						super();
						this.loader = new p2p_media_loader_core_1.HybridLoader(
							settings.loader
						);
						this.segmentManager =
							new segment_manager_1.SegmentManager(
								this.loader,
								settings.segments
							);
						Object.keys(p2p_media_loader_core_1.Events)
							.map(
								(eventKey) =>
									p2p_media_loader_core_1.Events[eventKey]
							)
							.forEach((event) =>
		...
		3: [
			function (require, module, exports) {
				...
				function initShakaPlayer(player, segmentManager) {
					...
					player
						.getNetworkingEngine()
						.registerRequestFilter((requestType, request) => {
				...
				function registerParserProxies() {
					...
					shaka.media.ManifestParser.registerParserByExtension(
						"mpd",
						manifest_parser_proxy_1.ShakaDashManifestParserProxy
					);
					...
					shaka.media.ManifestParser.registerParserByExtension(
						"m3u8",
						manifest_parser_proxy_1.ShakaHlsManifestParserProxy
					);
					shaka.media.ManifestParser.registerParserByMime(
				...
				function processNetworkRequest(
					...
					if (!request.p2pml) {
						return shaka.net.HttpXHRPlugin(
							uri,
							request,
							requestType,
							progressUpdated
					...
					let assetsStorage =
						segmentManager.getSettings().assetsStorage;
					let masterManifestUri;
					let masterSwarmId;
					if (
						assetsStorage !== undefined &&
						player.getNetworkingEngine().p2pml !== undefined &&
						player.getNetworkingEngine().p2pml.masterManifestUri !==
							undefined
					) {
						masterManifestUri =
							player.getNetworkingEngine().p2pml
								.masterManifestUri;
						masterSwarmId = utils_1.getMasterSwarmId(
							masterManifestUri,
					...
					if (
						...
						const responsePromise = (async () => {
							const asset = await assetsStorage.getAsset(
								uri,
								request.headers.Range,
								masterSwarmId
							);
							if (asset !== undefined) {
								...
								const response = await shaka.net.HttpXHRPlugin(
									uri,
									request,
									requestType,
									progressUpdated
			...
			{ "./manifest-parser-proxy": 4, "./utils": 7, debug: "debug" },
		],
		4: [
			function (require, module, exports) {
				"use strict";
				...
				class ShakaManifestParserProxy {
					constructor(originalManifestParser) {
						this.cache = new parser_segment_1.ParserSegmentCache(
							200
						);
					...
					isHls() {
						return (
							this.originalManifestParser instanceof
							shaka.hls.HlsParser
						);
					}
					isDash() {
					...
					start(uri, playerInterface) {
						...
						if (
							playerInterface.networkingEngine.p2pml === undefined
						) {
							playerInterface.networkingEngine.p2pml = {};
						}
						playerInterface.networkingEngine.p2pml.masterManifestUri =
							uri;
						return this.originalManifestParser
							.start(uri, playerInterface)
							.then((manifest) => {
								...
								for (const period of manifest.periods) {
									...
									for (const variant of period.variants) {
										if (
											variant.video != null &&
											processedStreams.indexOf(
												variant.video
											) == -1
										) {
											this.hookGetSegmentReference(
												variant.video
											);
					...
					hookGetSegmentReference(stream) {
						stream.getSegmentReferenceOriginal =
							stream.getSegmentReference;
						stream.getSegmentReference = (segmentNumber) => {
							this.cache.add(stream, segmentNumber);
							return stream.getSegmentReferenceOriginal(
								segmentNumber
							);
						};
						stream.getPosition = () => {
							if (this.isHls()) {
								if (stream.type === "video") {
									return this.manifest.periods[0].variants
										.reduce((a, i) => {
											if (
												i.video &&
												i.video.id &&
												!a.includes(i.video.id)
											) {
												a.push(i.video.id);
											}
											return a;
				...
				exports.ShakaDashManifestParserProxy =
				...
				exports.ShakaHlsManifestParserProxy =
			...
			function (require, module, exports) {
				...
				class ParserSegment {
					...
					static create(stream, position) {
						const ref =
							stream.getSegmentReferenceOriginal(position);
						if (!ref) {
							return undefined;
						}
						...
						const range =
							startByte || endByte
								...
								: undefined;
						const streamTypeCode = stream.type
							.substring(0, 1)
							.toUpperCase();
			...
			function (require, module, exports) {
				...
				class SegmentManager {
					constructor(loader, settings = {}) {
						...
						this.loader = loader;
						this.loader.on(
							p2p_media_loader_core_1.Events.SegmentLoaded,
							this.onSegmentLoaded
						);
						this.loader.on(
							p2p_media_loader_core_1.Events.SegmentError,
							this.onSegmentError
						);
						this.loader.on(
							p2p_media_loader_core_1.Events.SegmentAbort,
							this.onSegmentAbort
						);
					}
					async destroy() {
						if (this.requests.size !== 0) {
							console.error(
								"Destroying segment manager with active request(s)!"
							);
							for (const request of this.requests.values()) {
								this.reportError(
									request,
									"Request aborted due to destroy call"
								);
							}
							this.requests.clear();
						}
					...
					async load(parserSegment, manifestUri, playheadTime) {
						...
						const alreadyLoadedSegment =
							await this.loader.getSegment(
								lastRequestedSegment.id
							);
					...
					refreshLoad() {
						const lastRequestedSegment =
							this.segmentHistory[this.segmentHistory.length - 1];
						const safePlayheadTime =
							this.playheadTime > 0.1
								? this.playheadTime
								: lastRequestedSegment.start;
						...
						do {
							...
							sequence.length < this.settings.forwardSegmentCount
						);
						const masterSwarmId = utils_1.getMasterSwarmId(
							this.manifestUri,
							this.settings
						);
						...
						this.loader.load(
							loaderSegments,
							`${masterSwarmId}+${lastRequestedSegment.streamIdentity}`
						);
						return loaderSegments[lastRequestedSegmentIndex];
					}
					pushSegmentHistory(segment) {
						if (
							this.segmentHistory.length >=
							this.settings.maxHistorySegments
						) {
							this.debug("segment history auto shrink");
							this.segmentHistory.splice(
								0,
								this.settings.maxHistorySegments * 0.2
							);
						}
						if (
							...
							this.debug(
								"segment history reset due to playhead seek back"
							);
							this.segmentHistory.splice(0);
						}
					...
					reportSuccess(request, loaderSegment) {
						...
						if (
							...
							timeMs = Math.trunc(
								loaderSegment.data.byteLength /
									loaderSegment.downloadBandwidth
							);
						}
