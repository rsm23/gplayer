    ...
    */
    ...
    var captions = {
      ...
      toggle: function toggle(input) {
        ...
        if (typeof this.supported !== "undefined" && !this.supported.ui) {
          return;
        }
        ...
        if (active !== toggled) {
          ...
          if (!passive && typeof this.storage !== "undefined") {
    ...
    var ui = {
      ...
      toggleControls: function toggleControls(force) {
        ...
        if (controlsElement && this.config.hideControls) {
          ...
          var recentTouchSeek = this.touch && this.lastSeekTime + 3000 > Date.now(); // Show controls if force, loading, paused, button interaction, or recent seek, otherwise hide
    ...
    var Ads = /*#__PURE__*/function () {
      ...
      _createClass(Ads, [{
        ...
        value: function setupIMA() {
          ...
          google.ima.settings.setDisableCustomPlaybackForIOS10Plus(false); // We assume the adContainer is the video container of the plyr element that will house the ads
        ...
        value: function onAdEvent(event) {
          ...
          // MULAI modifikasi disini
          const adTogglePlay = (play) => {
            const classPressed = "plyr__control--pressed";
            const btn = this.player.ads.elements.container.querySelector("button[data-plyr='play']");
            if (btn) {
                if (play) {
                    btn.classList.add(classPressed);
                } else {
                    btn.classList.remove(classPressed);
                }
                btn.setAttribute("aria-pressed", play);
                this.player.lastSeekTime = Date.now();
            }
        };
        const updateAd = () => {
            container.classList.add("plyr--ad-loaded");
            ...
            container.classList.remove("plyr--ad-vpaid");
            if (ad.isLinear()) {
                container.classList.add("plyr--ad-linear");
            } else {
                const plyrAds = this.elements.container;
                const divAds = plyrAds.firstChild;
                if (ad.data.vpaid) {
                    container.classList.add("plyr--ad-vpaid");
                } else {
                    divAds.style.width = ad.getWidth() + "px";
                    divAds.style.height = ad.getHeight() + "px";
                    plyrAds.style.width = divAds.style.width;
                    plyrAds.style.height = divAds.style.height;
                    container.classList.add("plyr--ad-nonlinear");
                }
            }
        };
        // SELESAI modifikasi disini
        ...
        switch (event.type) {
            ...
            case google.ima.AdEvent.Type.VIDEO_CLICKED:
                updateAd();
                adTogglePlay(false);
                break;
            ...
            case google.ima.AdEvent.Type.RESUMED:
                adTogglePlay(true);
                // Set volume to match google ima sdk
                this.manager.setVolume(this.manager.getVolume());
                break;
            // SELESAI modifikasi disini
            ...
            case google.ima.AdEvent.Type.LOG:
                if (adData.adError) {
                    this.player.debug.warn(
                        `Non-fatal ad error: ${adData.adError.getMessage()}`
                    );
                }
                break;
        }
        ...
        value: function getThumbnail(url) {
          ...
          return new Promise(function (resolve) {
            fetch(url).then(function (response) {
              ...
              if (thumbnail.frames.length && !thumbnail.frames[0].text.startsWith('/') && !thumbnail.frames[0].text.startsWith('http://') && !thumbnail.frames[0].text.startsWith('https://')) {
    ...
    var Plyr = /*#__PURE__*/function () {
      ...
      _createClass(Plyr, [{
        ...
        set: function set(input) {
          ...
          if (updateStorage && typeof this.storage !== "undefined") {
            this.storage.set({
              quality: quality
            });
          }
